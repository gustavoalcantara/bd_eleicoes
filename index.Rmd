---
title: "A relação entre o IDHM e a votação presidencial de 2018 no estado de SP"
author: "Gustavo Casteletti de Alcântara"
date: "27/02/2022"
output:
  prettydoc::html_pretty:
    theme: cerulean
editor_options: 
  chunk_output_type: console
---
## Introdução
Seguindo a proposta do Case, utilizei, através da [Base dos Dados](https://basedosdados.org/), a tabela dos resultados das Eleições Brasileiras disponíveis [aqui](https://basedosdados.org/dataset/br-tse-eleicoes). 
O acesso e utilização do pacote [basedosdados](https://basedosdados.github.io/mais/api_reference_r/) no Rstudio, o download e limpeza inicial (seleção, filtragem de atributos e criação de variáveis) encontram-se [aqui](https://github.com/gustavoalcantara/bd_eleicoes/blob/master/script_download_limpeza_dataset.R)

### DF de trabalho e outras proposições
Com o df pronto para uso, é válido ressaltar que a tabela inicial possuía cerca de 23M de linhas. Se fosse necessário o download deste arquivo em minha máquina, faria a seleção e limpeza dos dados atráves do datalake do Google pela regra SQL que elaborei. Visualize-a [aqui](https://console.cloud.google.com/bigquery?sq=52786671863:dca4197316d34aca8d3f63ef74044d34):  
```{r include=FALSE}
dados <- read.csv('dados_eleicoes.csv')
```
### Visualização do df
A tabela a seguir é o dataframe que iremos trabalhar de agora em diante. São um pouco mais de 9k linhas e 10 colunas ao total. Veja:
```{r}
dplyr::glimpse(dados)
```
## Votação presidenciável no 1º Turno
A partir do df de trabalho, elaborei um gráfico simples de boxplot agrupando os municípios e a votação para cada partido: 
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
library(tidyverse)
```
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
dplyr::filter(dados,
              turno==1)|>
  dplyr::group_by(id_municipio)|>
  ggplot2::ggplot(aes(x=reorder(sigla_partido, porcentagem, FUN=median)
                      , y=porcentagem))+
  geom_boxplot() +
  coord_flip()+
  labs(x = "Partidos", y = "Porcentagem de Votos")
```

## O IDHM e sua relação com a votação
Como forma de trabalhar com mais de uma tabela ao mesmo tempo e realizar análises exploratórias que relacionam-se, é necessário ter em mente que um atributo ou mais que estejam presente nas tabelas sejam iguais. Assim, realizei um join (utilizando a biblioteca do dplyr) da tabela do IDHM disponível no site da [PNUD Brasil](https://www.br.undp.org/content/brazil/pt/home/idh0/rankings/idhm-municipios-2010.html) com a Id dos municipios de São Paulo. Porém, é necessário alterar o nome da variável 'Cod.IBGE' para 'id_municipio' para realizar o Join. Essa alteração deixei no script de manipulação dos dados [aqui]   
```{r include=FALSE}
idhm <- read.csv('idhm_sp.csv')
```
```{r include=FALSE}
idhm <- dplyr::rename(idhm, id_municipio = "Cod.IBGE")
```
```{r}
dplyr::glimpse(idhm)
```
### Join das tabelas
É necessário realizar a união dos dois dataframes. Para isso vou utilizar a função left_join() do dplyr. 
```{r include=FALSE}
dplyr::left_join(dados, 
                 idhm, 
                 by='id_municipio'
) -> dados_idhm
```
```{r include=FALSE}
dados_idhm |> 
  mutate(X.x = NULL,
         X.y = NULL) -> dados_idhm
```

```{r}
dplyr::glimpse(dados_idhm)
```
```{r include=FALSE}
write.csv(dados_idhm, 'idhm_dados.csv', row.names = TRUE) ->dados_idhm
```

## Análise do IDHM com a Porcentagem de voto
Índices são úteis para avaliar e compreender contextos diversos. É válido afirmar que o estudo do Índice de Desenvolvimento Humano Municipal é referencial de uma série de artigos, livros e discussões que relacionam-o com outras variáveis. Aqui propus relacioná-lo com a votação dos partidos nos municipios do estado de São Paulo. Para isso,   





